...
public class CreditOfferUpdateController {
    ...
    @FXML
    public void updateClientOfferButtonAction(ActionEvent event) {
    	...
    	if(checkFields()) {   		
    		...
    		Client client = new Client(	prevClient.getId(),
    		...
    		Credit credit = new Credit( prevCredit.getClientID(),
    		...
    		/**
    		 * Обновляем клиента и кредит в локальном списке
    		 */
    		if(Bank.clientList.contains(prevClient)) {
    			Bank.clientList.remove(prevClient);
    			Bank.clientList.add(client);
    			System.out.println("Клиент найден и изменён в локальной базе");
    		} else {
    			System.out.println("Клиент не найден в локальной базе");
    		}
    		
    		if(Bank.creditList.contains(prevCredit)) {
    			Bank.creditList.remove(prevCredit);
    			Bank.creditList.add(credit);
    			System.out.println("Кредит найден и изменён в локальной базе");
    		} else {
    			System.out.println("Кредит не найден в локальной базе");
    		}
    	
    		/**
    		 * Изменяем клиента в БД
    		 */
    		...
    		clientController.updateObject(client);
    		...
    		creditController.updateObject(credit);
    		creditController.closeDB();
    		
    	   	Stage stage = (Stage) addButtonAction.getScene().getWindow();
        	stage.close();
